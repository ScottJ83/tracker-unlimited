...
export default function MtgQuantityControls({ printingId, quantity = 0, compact = false }: Props) {
  ...
  return (
    <div className={compact ? "mtg-qty mtg-qty-compact" : "mtg-qty"}>
      <div className="mtg-qty-total">Owned {quantity}</div>
      <div className="mtg-qty-row">
        <button disabled={loading !== null} onClick={() => update("decrement")}>−</button>
        <strong>{quantity}</strong>
        <button disabled={loading !== null} onClick={() => update("increment")}>+</button>
      </div>
    </div>
  );
}
